...
export function CartItem({ item, index, onRemove }: CartItemProps) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--color-border)' }}>
      <div>
        <span style={{ fontWeight: 600 }}>{item.proteinName}</span>
        {item.starchName && (
          <span style={{ color: 'var(--color-text-secondary)', marginLeft: 8 }}>+ {item.starchName}</span>
        )}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span style={{ fontWeight: 700 }}>
          {new Intl.NumberFormat('en-UG', { style: 'currency', currency: 'UGX', minimumFractionDigits: 0 }).format(item.proteinPrice)}
        </span>
